...
def main_window():
    ...
    # Owners tab
    owner_tab = ctk.CTkFrame(tab_control)
    ...
    employee_tree.pack(fill=ctk.BOTH, expand=True)
    ...
    # Services tab
    service_tab = ctk.CTkFrame(tab_control)
    ...
    service_tree.pack(fill=ctk.BOTH, expand=True)
    ...
    # Repairs tab
    repair_tab = ctk.CTkFrame(tab_control)
    ...
    repair_tree.pack(fill=ctk.BOTH, expand=True)
...
def show_add_dialog(table_name, columns, tree):
    ...
    for i, column in enumerate(columns):
        label = ctk.CTkLabel(dialog, text=column.capitalize())
        label.grid(row=i, column=0, padx=10, pady=10)
        entry = ctk.CTkEntry(dialog)
        entry.grid(row=i, column=1, padx=10, pady=10)
        entries.append(entry)
    submit_button = ctk.CTkButton(dialog, text="Submit", command=submit)
...
def add_buttons(tab, tree, table_name, columns, primary_key_column):
    add_button = ctk.CTkButton(tab, text="Add", command=lambda: show_add_dialog(table_name, columns, tree))
    add_button.pack(side=ctk.LEFT, padx=10, pady=10)

    delete_button = ctk.CTkButton(tab, text="Delete",
    ...
    modify_button = ctk.CTkButton(tab, text="Modify",
                               command=lambda: show_modify_dialog(tab, table_name, primary_key_column,
                                                                  tree.item(tree.selection()[0])['values'][0], tree))
    modify_button.pack(side=ctk.LEFT, padx=10)
...
def show_modify_dialog(parent, table_name, primary_key_column, id_field, tree):
    ...
    for i, field in enumerate(fields):
        label = ctk.CTkLabel(top, text=f"{field.capitalize()}:")
        label.grid(row=i, column=0, sticky=ctk.W, padx=(10, 5), pady=(10, 0))

        entry = ctk.CTkEntry(top)
        entry.insert(0, selected_data[i + 1])
        entry.grid(row=i, column=1, sticky=ctk.E, padx=(5, 10), pady=(10, 0))
    ...
    modify_button = ctk.CTkButton(top, text="Modify", command=modify_record)
